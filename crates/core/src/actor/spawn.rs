...
use crate::mailbox;
use crate::pid::Pid;
use crate::types::{ErasedMessageHandler, VirtualActorSpec};
use crate::Runtime;
...
impl Runtime {
    ...
    {
        self.spawn_actor_link(handler).target()
    }

    pub fn spawn_actor_link<H, Fut>(&self, handler: H) -> crate::actor::capability::Link
    where
        H: FnOnce(mailbox::MailboxReceiver) -> Fut + Send + 'static,
        Fut: std::future::Future<Output = ()> + Send + 'static,
    {
        if self.is_load_shedding_active() {
            ...
            return crate::actor::capability::Link::new(0, vec![]);
        }
        let slab = &self.slab.0;
        let pid = slab.allocate();
        let (tx, rx) = mailbox::channel();
        self.mailboxes.0.insert(pid, tx.clone());
        self.telemetry
            .0
            .log_event(crate::telemetry::TelemetryEvent::ActorSpawned {
                pid,
                path: "".to_string(),
            });
        ...
        let mailboxes2 = self.mailboxes.0.clone();
        let supervisor2 = self.supervisor.0.clone();
        let slab2 = self.slab.0.clone();
        ...
        crate::actor::capability::Link::full(pid)
    }
    ...
    pub fn spawn_handler_link<H, Fut>(
        &self,
        handler: H,
        budget: usize,
    ) -> crate::actor::capability::Link
    where
        ...
        crate::RUNTIME.spawn(async move {
            ...
            let actor_handle = tokio::spawn(async move {
                ...
                while let Some(first_msg) = rx.recv().await {
                    rt_inner
                        .telemetry()
                        .log_event(crate::telemetry::TelemetryEvent::MessageReceived { pid });
                        ...
                        match rx.try_recv() {
                            Some(next_msg) => {
                                rt_inner.telemetry().log_event(
                                    crate::telemetry::TelemetryEvent::MessageReceived { pid },
                                );
    ...
    pub fn spawn_observed_handler(&self, _budget: usize) -> Pid {
        ...
        self.observers.0.insert(pid, vec.clone());
    ...
    pub(crate) fn ensure_virtual_actor_active(&self, pid: Pid) -> bool {
        ...
        let spec = if let Some((_, spec)) = self.virtual_specs.remove(&pid) {
            ...
            return self.mailboxes.0.contains_key(&pid);
        };
    ...
    {
        if let Some(entry) = self.observers.0.get(&pid) {
