...
impl Runtime {
    ...
    pub(crate) fn finalize_actor_exit(
        ...
        slab: &Arc<pid::SlabAllocator>,
        path_supervisors: &Arc<DashMap<String, Arc<super::supervisor::Supervisor>>>,
        rt: &Runtime,
        pid: Pid,
        ...
        match reason {
            mailbox::ExitReason::Normal => {
                rt.telemetry()
                    .log_event(crate::telemetry::TelemetryEvent::ActorStopped { pid });
            }
            _ => {
                rt.telemetry()
                    .log_event(crate::telemetry::TelemetryEvent::ActorCrashed {
                        pid,
    ...
    pub fn stop(&self, pid: Pid) {
        ...
        if self.mailboxes.0.remove(&pid).is_some() {
    ...
    pub fn supervise(
        ...
        self.supervisor.0.add_child(pid, spec);
    }

    pub fn link(&self, a: Pid, b: Pid) {
        self.supervisor.0.link(a, b);
    }

    pub fn unlink(&self, a: Pid, b: Pid) {
        self.supervisor.0.unlink(a, b);
    }
    ...
    pub(crate) fn handle_exit_internal(&self, pid: Pid) {
        ...
        self.behavior_versions.remove(&pid);
        self.behavior_history.remove(&pid);
        self.observers.0.remove(&pid); // remove the pid from its parent's child list (if any)
